...
class MCPAcademicServer:
    ...
    def call_tool(self, tool_name: str, arguments: Dict[str, Any]) -> Dict[str, Any]:
        ...
        """
        # --------------------------------------------------------------------------
        # TASK 2.1 COMPLETED: GỌI TOOL VÀ ĐÓNG GÓI PHẢN HỒI JSON-RPC
        ...
        # --------------------------------------------------------------------------
        content_str = dispatch_tool_call(tool_name, arguments)
...
if __name__ == "__main__":
    ...
    if sched_tool and not sched_tool.get("parameters", {}).get("properties"):
        print("⏳ [TASK 1.2 INCOMPLETE]: Tool 'schedule_appointment' chưa được định nghĩa properties trong 'src/tools.py'.")
    else:
        print("✅ [TASK 1.2 COMPLETED]: Tool 'schedule_appointment' đã có schema đầy đủ.")

    # Kiểm tra trạng thái Task 2.1 (call_tool)
    test_result = server.call_tool("academic_query", {"student_id": "SV2026001"})
    if not test_result:
        print("⏳ [TASK 2.1 INCOMPLETE]: Hàm call_tool() đang trả về rỗng.")
    else:
        print(f"✅ [TASK 2.1 COMPLETED]: Test dispatch tool 'academic_query' thành công:")
